...
全称是 [Node.js API](https://nodejs.org/api/n-api.html){:target="_blank"}，主要是为了将 addon 编程与具体的 JavaScript 解释器（V8）API 解耦，抽象出一层与解释器无关的 API。换句话说就是我们不再用具体的 V8 API 了，而是 Node.js 封装的 N-API 去做原本用 V8 API 完成的事。这是一套 C API，官方也提供了相应的 [C++ wrapper](https://github.com/nodejs/node-addon-api){:target="_blank"}
